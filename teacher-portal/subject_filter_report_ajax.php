...
if ($action === 'getSubjects') {
    ...
    ob_end_clean();
    echo json_encode(['subjects' => $all_subjects]);
}
    ...
    if ($action === 'getClasses') {
        ...
        if (!empty($selected_subject)) {
            ...
            foreach ($all_result_rows as $row) {
                if($row['subject_id']== $selected_subject){
                    $classes[] = $row['class'];
                }
            }
    ...
    elseif ($action === 'viewReport') {
        ...
        if (!empty($selected_subject)) {
            ...
            if (!empty($selected_class) && !empty($selected_student)){
                ...
                foreach ($all_result_rows as $row)  {
                    $temp = [];
                    if($row['subject_id']==$selected_subject && $row['student_id']==$selected_student){
                        if(!(in_array($row['student_id'],$student_ids))){
                            $student_ids[] = $row['student_id'];
                            $temp["student_id"] = $row['student_id'];
                            $temp["student_name"] = $row['student_name'];
        ...
        $output .= '<table border="1" cellpadding="5" cellspacing="0"><thead><tr><th>Id</th><th>Student Name</th>';
        $tabletitlerow[] = "Id";
        ...
        foreach ( $students_table as $student_row) {
            $output .= '<tr><td>' . htmlspecialchars($student_row["student_id"]) . '</td><td>' . htmlspecialchars($student_row["student_name"]) . '</td>';
            ...
            foreach ($subject_data as $subject) {
                ...
                $output .= '<td>' . ($marks ? htmlspecialchars($marks) : '-') . '</td>';
            }
